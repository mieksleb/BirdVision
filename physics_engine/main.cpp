...
//    energy_file << "  in the total energy.\n";
    ...
    for ( step = 0; step <= step_num; step++ ) {
        ...
        else
        {
            ...
            if ( !crossed_net && pos[1] > 0 ) {
                if ( pos[2] < net_height_min ) {
                    hit_net = true; // failed to clear net
                    impact_pos = pos; // record impact with net
        ...
        {
            WriteVector2Line(traj_file, pos, width, prec, false);
            WriteVector2Line(traj_file, vel, width, prec, false);
            WriteVector2Line(traj_file, omega, width, prec, true);
    ...
    event_file << fixed << setw(width) << left << hit_net << setw(width) << right
               << setw(width) << left << is_out << setw(width) << right;
    WriteVector2Line(event_file, impact_pos, width, prec, true);
    WriteVector2Line(traj_file, pos, width, prec, false);
    WriteVector2Line(traj_file, vel, width, prec, false);
    WriteVector2Line(traj_file, omega, width, prec, true);
    traj_file.close();
